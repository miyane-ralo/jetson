...
class Processor():
    def __init__(self, model):
        print('setting up trt processor')
        # load tensorrt engine
        TRT_LOGGER = trt.Logger(trt.Logger.INFO)
        TRTbin = '{0}/{1}'.format(os.path.dirname(__file__), model)
    ...
    def extract_boxes(self, output, conf_thres=0.4):
        """
        ...
        for out, grid, stride, anchor in zip(scaled, grids, self.strides, self.anchor_grid):
            ...
            out[..., 5:] = out[..., 4:5] * out[..., 5:]
            out = out.reshape((1, 3 * width * height, 10))
            z.append(out)
    ...
    def cls_process(self, outputs, conf_thres=0.25):
        """
    ...
    def non_max_suppression(self, boxes, confs, classes, iou_thres=0.45):
        x1 = boxes[:, 0]
        y1 = boxes[:, 1]
        x2 = boxes[:, 2]
        y2 = boxes[:, 3]
    ...
    '''def _nms(self, pred, iou_thres=0.6):
        boxes = self.xywh2xyxy(pred[..., 0:4])
        # best class only
        confs = np.amax(pred[:, 5:], 1, keepdims=True)
        print(boxes.shape,type(boxes),confs.shape)
        classes = np.argmax(pred[:, 5:], axis=-1)
        n_n = nms(boxes,confs,iou_thres)
        boxes = boxes[n_n]
        confs = confs[n_n]
        classes = classes[n_n]
        return boxes, confs, classes'''
